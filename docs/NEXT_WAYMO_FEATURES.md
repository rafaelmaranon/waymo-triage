...
**남은 구현 작업**:
1. `waymoLidarWorker.ts` — init에서 seg parquet `readAllRows()` → timestamp Map 구축, loadBatch에서 timestamp lookup + per-point label 추출
2. range image → point cloud 변환 과정에서 seg range image의 동일 좌표 위치에서 per-point label 추출
...
5. ~~`computePointColor()` — manifest의 `semanticPalette`에서 팔레트 resolve~~ → **완료** (4de9fc0: optional `semanticPalette` param + WeakMap HSL cache, `instanceColor()`도 동일 파라미터화)
...
- 198프레임 중 20~30프레임만 라벨 존재 (매 5프레임 간격, TOP 센서 only)
...
- **TOP 센서만 라벨 있음** — UI에서 segment 모드 시 라벨 없는 센서 포인트는 fallback 색상(gray) 표시 또는 투명도 낮춤
...
```
✅ 0. 선행작업 (완료)        — DatasetManifest 확장, palette, upng-js, 타입 선언, palette 파라미터화
→ 1. lidar_segmentation     — worker init 전량 로딩 + timestamp Map + per-point label
